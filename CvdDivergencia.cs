...
namespace CvdDivergencia
{
    ...
    public class CvdDivergencia : Indicator
    {
        ...
        //
        //  Itera por todos os swing highs anteriores dentro do MaxDistanciaBars
        //  e da mesma sessão de trading, para encontrar o par mais recente que
        //  forma uma divergência válida. Isto permite detetar divergências mesmo
        //  quando existem swings intermédios entre os dois topos relevantes.
        // ====================================================================

        private void TryAddHighDiv(SwingPoint curr)
        {
            // Limpar qualquer divergência High anterior para este Bar2 antes de reavaliar
            _divs.RemoveAll(d => d.Bar2 == curr.Bar &&
                (d.Type == DivType.ExaustaoHigh || d.Type == DivType.AbsorcaoHigh));

            // Iterar do penúltimo swing para o mais antigo
            // A lista _highsNQ está ordenada por Bar crescente, pelo que podemos parar
            // assim que a distância excede MaxDistanciaBars.
            for (int i = _highsNQ.Count - 2; i >= 0; i--)
            {
                var prev = _highsNQ[i];

                // Demasiado distante — todos os anteriores estão ainda mais longe
                if (curr.Bar - prev.Bar > MaxDistanciaBars) break;

                // CVD é resetado a 0 em cada sessão; valores de sessões diferentes não são comparáveis
                if (curr.Time.Date != prev.Time.Date) break;

                // Diferença de preço insuficiente para ser considerado divergência significativa
                if (Math.Abs(curr.Price - prev.Price) < MinDiferencaPreco) continue;
        ...
        //
        //  Itera por todos os swing lows anteriores dentro do MaxDistanciaBars
        //  e da mesma sessão de trading, para encontrar o par mais recente que
        //  forma uma divergência válida.
        // ====================================================================

        private void TryAddLowDiv(SwingPoint curr)
        {
            // Limpar qualquer divergência Low anterior para este Bar2 antes de reavaliar
            _divs.RemoveAll(d => d.Bar2 == curr.Bar &&
                (d.Type == DivType.ExaustaoLow || d.Type == DivType.AbsorcaoLow));

            // Iterar do penúltimo swing para o mais antigo
            for (int i = _lowsNQ.Count - 2; i >= 0; i--)
            {
                var prev = _lowsNQ[i];
                ...
                DivType? type = null;
                if ( priceLower && !cvdLower) type = DivType.ExaustaoLow;
                if (!priceLower &&  cvdLower) type = DivType.AbsorcaoLow;
                if (type == null) continue;
